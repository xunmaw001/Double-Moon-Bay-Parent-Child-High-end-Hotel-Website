		...
		<script type="text/javascript">
			var app = new Vue({
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "fangxingjieshao"){
				...
				methods: {
					...
					getDetail(id){
						return new Promise((resolve) => {
							goodsInfo('fangxingjieshao',id, (res) => {
								var scripts = [
					...
					async onStoreTap(){
						...
						var data = {
							...
							data.name = this.detail.biaoti
							data.picture = this.detail.kefangzhaopian
						storeUpAdd("fangxingjieshao", data, (res) => {
							if(res&&res.code==0){
								alert("收藏成功")
							}
						})
					...
					onThumbsUp(type){
						thumbsUp("fangxingjieshao", this.detail.id, type, (res) => {
							if(res&&res.code==0){
								if(type==1){
									alert("点赞成功")
					...
					getComments(){
						commentList("fangxingjieshao", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("fangxingjieshao", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
