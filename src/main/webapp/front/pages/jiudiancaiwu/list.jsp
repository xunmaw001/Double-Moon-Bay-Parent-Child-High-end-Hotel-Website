						...
						<div :class="group?'col-md-9 col-sm-12 col-xs-12':'col-md-12 col-sm-12 col-xs-12'" >
							...
							<div style="display: flex;justify-content: flex-end;">
								<div class="input-group col-lg-3 col-md-12 col-sm-12" style="margin-left: 20px;margin-bottom: 20px">
									<input v-model="nianfen" type="text" class="form-control" placeholder="请输入年份">
									<span class="input-group-addon" style="background-color: #FE4847;color: #fff;cursor: pointor" @click="searchnianfenTap"><i class="fa fa-search"></i></span>
								</div>
								<div class="input-group col-lg-3 col-md-12 col-sm-12" style="margin-left: 20px;margin-bottom: 20px">
									<input v-model="yuefen" type="text" class="form-control" placeholder="请输入月份">
									<span class="input-group-addon" style="background-color: #FE4847;color: #fff;cursor: pointor" @click="searchyuefenTap"><i class="fa fa-search"></i></span>
								</div>
							</div>
		...
		<script type="text/javascript">
			...
			var app = new Vue({
				...
				methods: {
					searchnianfenTap(){
						this.page.page = 1
						this.searchname = "nianfen"
						this.searchval = this.nianfen
						this.getList(this.searchname, this.searchval)
					},
					searchyuefenTap(){
						this.page.page = 1
						this.searchname = "yuefen"
						this.searchval = this.yuefen
						this.getList(this.searchname, this.searchval)
					},
					onPageTap(url) {
					...
					getList(searchname, searchval) {
						return new Promise((resolve) => {
							goodsList('jiudiancaiwu',this.page.page, this.page.limit, searchname, searchval, (res) => {
								if (res.code == 0) {
									this.$nextTick(() => {
