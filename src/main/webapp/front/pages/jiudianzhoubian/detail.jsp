							...
							<div class="row">
								...
								<div class="col-md-7 col-sm-7 col-xs-12 shop-list shop-details">
									<div class="product-content">
										<div class="price">
											<span>景点名称：{{detail.jingdianmingcheng}}</span>
										</div>
										<div class="price">
											<span>景点类型：{{detail.jingdianleixing}}</span>
										</div>
										<div class="price">
											<span>开放时间：{{detail.kaifangshijian}}</span>
										</div>
										<div class="price">
											<span>景点地址：{{detail.jingdiandizhi}}</span>
										</div>
											  ...
											  <div class="tab-content">
												<div role="tabpanel" class="tab-pane active" id="tab-desc">
													<div class="product-tab-desc">
														<p>景点介绍</p>
														<div class="product-page-desc" v-html="detail.jingdianjieshao"></div>
													</div>
												</div>
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "jiudianzhoubian"){
				...
				methods: {
					...
					addCart(){
						...
						cartSave({
							...
							goodname: this.detail.jingdianmingcheng,
					...
					getDetail(id){
						return new Promise((resolve) => {
							goodsInfo('jiudianzhoubian',id, (res) => {
								var scripts = [
								...
								this.$nextTick(() => {
									...
									var arr = this.detail.jingdianfengmian.split(',')
									this.swiperList = []
									arr.forEach(item => {
					...
					async onStoreTap(){
						...
						var data = {
							...
							data.name = this.detail.jingdianmingcheng
							data.picture = this.detail.jingdianfengmian
						storeUpAdd("jiudianzhoubian", data, (res) => {
							if(res&&res.code==0){
								alert("收藏成功")
							}
						})
					...
					onThumbsUp(type){
						thumbsUp("jiudianzhoubian", this.detail.id, type, (res) => {
							if(res&&res.code==0){
								if(type==1){
									alert("点赞成功")
					...
					getComments(){
						commentList("jiudianzhoubian", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("jiudianzhoubian", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
