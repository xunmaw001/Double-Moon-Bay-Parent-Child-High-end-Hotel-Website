	...
	<body>
						...
						<div :class="group?'col-md-9 col-sm-12 col-xs-12':'col-md-12 col-sm-12 col-xs-12'" >
							...
							<div style="display: flex;justify-content: flex-end;">
								<div class="input-group col-lg-3 col-md-12 col-sm-12" style="margin-left: 20px;margin-bottom: 20px">
									<input v-model="jingdianmingcheng" type="text" class="form-control" placeholder="请输入景点名称">
									<span class="input-group-addon" style="background-color: #FE4847;color: #fff;cursor: pointor" @click="searchjingdianmingchengTap"><i class="fa fa-search"></i></span>
								</div>
								<div class="input-group col-lg-3 col-md-12 col-sm-12" style="margin-left: 20px;margin-bottom: 20px">
									<input v-model="jingdianleixing" type="text" class="form-control" placeholder="请输入景点类型">
									<span class="input-group-addon" style="background-color: #FE4847;color: #fff;cursor: pointor" @click="searchjingdianleixingTap"><i class="fa fa-search"></i></span>
								</div>
							</div>
										...
										<div class="row" style="display: flex;flex-wrap: wrap;">
											...
											<div class="single-product-box col-md-3 col-sm-4" v-for="(item, i) in list" :key="i" @click="onPageTap(`./detail.jsp?id=${item.id}`)">
												<div class="single-product">
													<div class="product-img" v-if="item.jingdianfengmian">
														<img :style="{height:group?'200px':'300px'}" :src="item.jingdianfengmian.split(',')[0]" alt="" />
													</div>
														<h3>{{item.jingdianmingcheng}}</h3>
													</div>
												</div>
											</div>
											<!-- single-product end -->
		...
		<script type="text/javascript">
			...
			var app = new Vue({
				...
				methods: {
					searchjingdianmingchengTap(){
						this.page.page = 1
						this.searchname = "jingdianmingcheng"
						this.searchval = this.jingdianmingcheng
						this.getList(this.searchname, this.searchval)
					},
					searchjingdianleixingTap(){
						this.page.page = 1
						this.searchname = "jingdianleixing"
						this.searchval = this.jingdianleixing
						this.getList(this.searchname, this.searchval)
					},
					onPageTap(url) {
					...
					getList(searchname, searchval) {
						return new Promise((resolve) => {
							goodsList('jiudianzhoubian',this.page.page, this.page.limit, searchname, searchval, (res) => {
								if (res.code == 0) {
									this.$nextTick(() => {
				...
				async created() {
					...
					var searchname = 'jingdianmingcheng'
