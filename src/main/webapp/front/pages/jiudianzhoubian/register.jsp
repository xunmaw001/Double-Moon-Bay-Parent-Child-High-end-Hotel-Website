					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="login-content login-margin">
								<h2 class="login-title">酒店周边注册</h2>
								<p>Create your own Unicase account.</p>
								<div class="form">
									<label>景点名称</label>
									<input type="text" v-model="jingdianmingcheng"/>
									<label>开放时间</label>
									<input type="text" v-model="kaifangshijian"/>
									<label>景点地址</label>
									<input type="text" v-model="jingdiandizhi"/>
									<label>赞</label>
			...
			var app = new Vue({
				...
				methods: {
					...
					if(!isIntNumer(this.crazilynum)){
						...
						var data = {
							"jingdianmingcheng": this.jingdianmingcheng,
							"kaifangshijian": this.kaifangshijian,
							"jingdiandizhi": this.jingdiandizhi,
							"thumbsupnum": this.thumbsupnum,
							"crazilynum": this.crazilynum,
						}
						register('jiudianzhoubian', data, function(res){
							if(res.code==0){
								alert('注册成功')
