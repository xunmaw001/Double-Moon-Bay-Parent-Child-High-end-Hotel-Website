		...
		<script type="text/javascript">
			var app = new Vue({
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "discusskefangxinxi"){
				...
				methods: {
					...
					getDetail(id){
						return new Promise((resolve) => {
							goodsInfo('discusskefangxinxi',id, (res) => {
								var scripts = [
					...
					getComments(){
						commentList("discusskefangxinxi", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("discusskefangxinxi", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
