		...
		<script type="text/javascript">
			var app = new Vue({
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "discussqinzihuodong"){
				...
				methods: {
					...
					getDetail(id){
						return new Promise((resolve) => {
							goodsInfo('discussqinzihuodong',id, (res) => {
								var scripts = [
					...
					getComments(){
						commentList("discussqinzihuodong", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("discussqinzihuodong", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
