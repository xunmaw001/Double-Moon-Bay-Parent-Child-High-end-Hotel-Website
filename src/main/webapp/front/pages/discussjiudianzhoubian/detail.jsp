		...
		<script type="text/javascript">
			var app = new Vue({
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "discussjiudianzhoubian"){
				...
				methods: {
					...
					getDetail(id){
						return new Promise((resolve) => {
							goodsInfo('discussjiudianzhoubian',id, (res) => {
								var scripts = [
					...
					getComments(){
						commentList("discussjiudianzhoubian", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("discussjiudianzhoubian", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
