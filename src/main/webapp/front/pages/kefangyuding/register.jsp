    ...
    <body>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="login-content login-margin">
								<h2 class="login-title">客房预订注册</h2>
								<p>Create your own Unicase account.</p>
								<div class="form">
									<label>客房类型</label>
									<input type="text" v-model="kefangleixing"/>
									<label>客房类别</label>
									<input type="text" v-model="kefangleibie"/>
									<label>门市价</label>
									<input type="text" v-model="menshijia"/>
									<label>剩余房间</label>
									<input type="text" v-model="shengyufangjian"/>
									<label>预定数量</label>
									<input type="text" v-model="yudingshuliang"/>
									<label>入住天数</label>
									<input type="text" v-model="ruzhutianshu"/>
									<label>实付金额</label>
									...
									<label>联系电话</label>
									<input type="text" v-model="lianxidianhua"/>
		...
		<script type="text/javascript">
			var app = new Vue({
				...
				methods: {
					userRegister(){
					if(!isIntNumer(this.yudingshuliang)){
						alert(`预定数量应输入整数`);
						return
					}
					if(!isIntNumer(this.ruzhutianshu)){
						alert(`入住天数应输入整数`);
						return
					}
						var data = {
							"kefangleixing": this.kefangleixing,
							"kefangleibie": this.kefangleibie,
							"menshijia": this.menshijia,
							"shengyufangjian": this.shengyufangjian,
							"yudingshuliang": this.yudingshuliang,
							"ruzhutianshu": this.ruzhutianshu,
							"shifujine": this.shifujine,
							"yonghuming": this.yonghuming,
							"xingming": this.xingming,
							"lianxidianhua": this.lianxidianhua,
						}
						register('kefangyuding', data, function(res){
							if(res.code==0){
								alert('注册成功')
