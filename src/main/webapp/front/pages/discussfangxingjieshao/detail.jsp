							...
							<div class="row">
								...
								<div class="col-md-7 col-sm-7 col-xs-12 shop-list shop-details">
									<div class="product-content">
										<div class="price">
											<span>关联表id：{{detail.refid}}</span>
										</div>
										<div class="price">
											<span>评论内容：{{detail.content}}</span>
										</div>
										<div class="price">
		...
		<script type="text/javascript">
			var app = new Vue({
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "discussfangxingjieshao"){
				...
				methods: {
					...
					getDetail(id){
						return new Promise((resolve) => {
							goodsInfo('discussfangxingjieshao',id, (res) => {
								var scripts = [
					...
					getComments(){
						commentList("discussfangxingjieshao", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("discussfangxingjieshao", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
