					...
					<div>
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input v-model="detail.kefangbianhao" type="text" name="" placeholder="请输入客房编号" autocomplete="off" class="layui-input" readonly="readonly">
							</div>
						</div>
						<div class="layui-form-item">
						...
						<div class="layui-form-item">
							<label class="layui-form-label">请选择客房类别</label>
							<div class="layui-input-block">
								<select id="kefangleibie" name="interest" lay-filter="kefangleibie">
									<option :value="item" v-for="(item,index) in kefangleibieOptions">{{item}}</option>
								</select>
							</div>
						</div>
						<div class="layui-form-item">
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
							  <input v-model="detail.kefangmianji" type="text" name="title" lay-verify="title" autocomplete="off" placeholder="请输入客房面积" class="layui-input">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">客房楼层</label>
							<div class="layui-input-block">
							  <input v-model="detail.kefanglouceng" type="text" name="title" lay-verify="title" autocomplete="off" placeholder="请输入客房楼层" class="layui-input">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">请选择免费宽带</label>
							<div class="layui-input-block">
								<select id="mianfeikuandai" name="interest" lay-filter="mianfeikuandai">
									<option :value="item" v-for="(item,index) in mianfeikuandaiOptions">{{item}}</option>
								</select>
							</div>
						</div>
						<div class="layui-form-item">
		...
		<script type="text/javascript">
			var app = new Vue({
				...
				data: {
					...
					crosstableobj: {},
					detail: {
						kefangbianhao: (new Date()).getTime()+"",
						kefangleixing: "",
						kefangleibie: "",
						kefangzhaopian: "",
						kefangmianji: "",
				...
				async created() {
					...
					if(tablename){
						...
						for(var key in cross){
							...
							if("kefanglouceng" == key){
								this.detail[key] = cross[key]
							}
							if("mianfeikuandai" == key){
								this.detail[key] = cross[key]
							}
							if("menshijia" == key){
								this.detail[key] = cross[key]
					...
					this.kefangleibieOptions = "含早餐,含中餐,含晚餐,含三餐".split(',')
					this.mianfeikuandaiOptions = "有线宽带,无线宽带".split(',')
				},
				methods: {
					onPageTap(url) {
					...
					onConfirmTap(){
					if(!isIntNumer(this.detail.menshijia)){
						alert(`门市价应输入整数`);
						return
					}
					if(!isIntNumer(this.detail.shengyufangjian)){
						...
						addFun("kefangxinxi", this.detail, (res) => {
							if(res&&res.code==0){
								alert("提交成功")
								window.history.go(-1);
							}
 			...
 			$(function(){
				...
				layui.use(['form','layedit'], function(){
					...
					form.on('select(kefangleibie)', function(data){
						app.detail.kefangleibie = data.value
					});
					form.on('select(mianfeikuandai)', function(data){
						app.detail.mianfeikuandai = data.value
					});
				});
			});
		</script>
