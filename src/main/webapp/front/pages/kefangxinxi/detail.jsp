							...
							<div class="row">
								...
								<div class="col-md-7 col-sm-7 col-xs-12 shop-list shop-details">
									<div class="product-content">
										<div class="price">
											<span>客房编号：{{detail.kefangbianhao}}</span>
										</div>
										<div class="price">
											<span>客房类型：{{detail.kefangleixing}}</span>
										</div>
										<div class="price">
											<span>客房类别：{{detail.kefangleibie}}</span>
										</div>
										<div class="price">
											<span>客房面积：{{detail.kefangmianji}}</span>
										</div>
										<div class="price">
											<span>客房楼层：{{detail.kefanglouceng}}</span>
										</div>
										<div class="price">
											<span>免费宽带：{{detail.mianfeikuandai}}</span>
										</div>
										<div class="price">
											<span>门市价：{{detail.menshijia}}</span>
										</div>
										<div class="price">
											<span>剩余房间：{{detail.shengyufangjian}}</span>
										</div>
										<div class="s-p-rating">
											<span class="review-no" @click="onThumbsUp(1)" style="cursor: pointer">赞：{{detail.thumbsupnum}}</span>
											<span class="review-no add-review" @click="onThumbsUp(2)" style="cursor: pointer">踩：{{detail.crazilynum}}</span>
										</div>
										...
										<div v-for="item in buttons">
											<div class="product-action" v-if="item=='增加'">
												<div class="button-cart">
													<button @click="onCrossTap('zengjiafangyuan')">增加</button>
												</div>
											</div>
										</div>
										
										<div v-for="item in buttons">
											<div class="product-action" v-if="item=='减少'">
												<div class="button-cart">
													<button @click="onCrossTap('jianshaofangyuan')">减少</button>
												</div>
											</div>
										</div>
										
										<div v-for="item in buttons">
											<div class="product-action" v-if="item=='预定'">
												<div class="button-cart">
													<button @click="onCrossTap('kefangyuding')">预定</button>
												</div>
											</div>
										</div>
									...
									<div class="product-tabs">
											<div>
											  ...
											  <ul class="pro-details-tab" role="tablist">
												<li role="presentation" class="active"><a href="#tab-desc" aria-controls="tab-desc" role="tab" data-toggle="tab">详情</a></li>
												<li role="presentation"><a href="#page-comments" aria-controls="page-comments" role="tab" data-toggle="tab">评论({{comments.length}}条评论)</a></li>
											  </ul>
											  <!-- Tab panes -->
											  <div class="tab-content">
												<div role="tabpanel" class="tab-pane active" id="tab-desc">
													<div class="product-tab-desc">
														<p>客房介绍</p>
														<div class="product-page-desc" v-html="detail.kefangjieshao"></div>
													</div>
												</div>
													...
													<div class="product-tab-desc">
														<div class="product-page-comments">
															...
															<div class="review-form-wrapper">
																<h3>您的评论</h3>
																<form @submit.prevent="onCommentTap" action="#">
																	<textarea v-model="comment" id="product-message" cols="30" rows="10" placeholder="请填写评论"></textarea>
				...
				methods: {
					...
					addCart(){
						...
						cartSave({
							...
							goodname: this.detail.menshijia,
						...
						return new Promise((resolve) => {
							goodsInfo('kefangxinxi',id, (res) => {
								...
								this.$nextTick(() => {
									...
									var arr = this.detail.kefangzhaopian.split(',')
									this.swiperList = []
									arr.forEach(item => {
					...
					onCrossTap(tablename){
						...
						window.location.href = `../${tablename}/add.jsp?tablename=kefangxinxi`
					},
					async onStoreTap(){
						...
						var data = {
							...
							data.picture = this.detail.kefangzhaopian
							data.name = this.detail.menshijia
						storeUpAdd("kefangxinxi", data, (res) => {
							if(res&&res.code==0){
								alert("收藏成功")
							}
						})
					...
					onThumbsUp(type){
						thumbsUp("kefangxinxi", this.detail.id, type, (res) => {
							if(res&&res.code==0){
								if(type==1){
									alert("点赞成功")
					...
					getComments(){
						commentList("kefangxinxi", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("kefangxinxi", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
