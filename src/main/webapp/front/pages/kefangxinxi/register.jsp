    ...
    <body>
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="login-content login-margin">
								<h2 class="login-title">客房信息注册</h2>
								<p>Create your own Unicase account.</p>
								<div class="form">
									<label>客房编号</label>
									<input type="text" v-model="kefangbianhao"/>
									<label>客房面积</label>
									<input type="text" v-model="kefangmianji"/>
									<label>客房楼层</label>
									<input type="text" v-model="kefanglouceng"/>
									<label>门市价</label>
		...
		<script type="text/javascript">
			var app = new Vue({
				...
				methods: {
					userRegister(){
					if(!isIntNumer(this.menshijia)){
						alert(`门市价应输入整数`);
						return
					}
					if(!isIntNumer(this.shengyufangjian)){
						alert(`剩余房间应输入整数`);
						return
					}
					...
					if(!isIntNumer(this.crazilynum)){
						...
						var data = {
							"kefangbianhao": this.kefangbianhao,
							"kefangmianji": this.kefangmianji,
							"kefanglouceng": this.kefanglouceng,
							"menshijia": this.menshijia,
							"shengyufangjian": this.shengyufangjian,
							"thumbsupnum": this.thumbsupnum,
							"crazilynum": this.crazilynum,
						}
						register('kefangxinxi', data, function(res){
							if(res.code==0){
								alert('注册成功')
