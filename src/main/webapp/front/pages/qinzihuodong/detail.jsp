							...
							<div class="row">
								...
								<div class="col-md-7 col-sm-7 col-xs-12 shop-list shop-details">
									<div class="product-content">
										...
										<div class="price">
											<span>活动地点：{{detail.huodongdidian}}</span>
										</div>
										<div class="price">
										...
										<div v-for="item in buttons">
											<div class="product-action" v-if="item=='购买'">
												<div class="button-cart">
													<button @click="onCrossTap('menpiaoyuding')">购买</button>
												</div>
											</div>
										</div>
											  ...
											  <div class="tab-content">
												<div role="tabpanel" class="tab-pane active" id="tab-desc">
													<div class="product-tab-desc">
														<p>活动详情</p>
														<div class="product-page-desc" v-html="detail.huodongxiangqing"></div>
													</div>
												</div>
				...
				async created() {
					...
					if(typeof frontmenus == "object"){
						frontmenus.forEach(item => {
							if(item.menutable == "qinzihuodong"){
				...
				methods: {
					...
					addCart(){
						...
						cartSave({
							...
							goodname: this.detail.huodongmingcheng,
						...
						return new Promise((resolve) => {
							goodsInfo('qinzihuodong',id, (res) => {
								...
								this.$nextTick(() => {
									...
									var arr = this.detail.huodongfengmian.split(',')
									this.swiperList = []
									arr.forEach(item => {
					...
					onCrossTap(tablename){
						...
						window.location.href = `../${tablename}/add.jsp?tablename=qinzihuodong`
					},
					async onStoreTap(){
						...
						var data = {
							...
							data.name = this.detail.huodongmingcheng
							data.picture = this.detail.huodongfengmian
						storeUpAdd("qinzihuodong", data, (res) => {
							if(res&&res.code==0){
								alert("收藏成功")
							}
						})
					...
					onThumbsUp(type){
						thumbsUp("qinzihuodong", this.detail.id, type, (res) => {
							if(res&&res.code==0){
								if(type==1){
									alert("点赞成功")
					...
					getComments(){
						commentList("qinzihuodong", this.detail.id, (res) => {
							if(res&&res.code==0){
					...
					async onCommentTap(){
						...
						if(user){
							...
							addComment("qinzihuodong", data, (res) => {
								if(res&&res.code==0){
									alert("评论成功")
									this.getComments()
								}
