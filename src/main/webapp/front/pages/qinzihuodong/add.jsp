			...
			<div class="form-box layui-form" lay-filter="layform">
				<div class="form-control form-content">
					...
					<div>
						...
						<div class="layui-form-item">
							<label class="layui-form-label">请选择活动类型</label>
							<div class="layui-input-block">
								<select id="huodongleixing" name="interest" lay-filter="huodongleixing">
									<option :value="item" v-for="(item,index) in huodongleixingOptions">{{item}}</option>
								</select>
							</div>
						</div>
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<button type="button" class="layui-btn" id="huodongfengmianupload-img" v-if="!detail.huodongfengmian">上传图片</button>
								<div class="layui-upload-list" v-if="detail.huodongfengmian">
									<img class="layui-upload-img" id="huodongfengmianimg-temp" :src="detail.huodongfengmian?detail.huodongfengmian.split(',')[0]:''">
									<p id="demoText"></p>
								</div>
							</div>
						</div>
						...
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
							  <input v-model="detail.huodongdidian" type="text" name="title" lay-verify="title" autocomplete="off" placeholder="请输入活动地点" class="layui-input">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">开始时间</label>
							<div class="layui-input-block">
								<input type="text" name="" placeholder="请输入开始时间" autocomplete="off" class="layui-input" id="kaishishijian">
							</div>
						</div>
						<div class="layui-form-item">
							<label class="layui-form-label">结束时间</label>
							<div class="layui-input-block">
								<input type="text" name="" placeholder="请输入结束时间" autocomplete="off" class="layui-input" id="jieshushijian">
							</div>
						</div>
				...
				async created() {
					...
					if(tablename){
						...
						for(var key in cross){
							...
							if("huodongfengmian" == key){
								this.detail[key] = cross[key]
							}
							if("menpiaojiage" == key){
								this.detail[key] = cross[key]
							}
							if("huodongdidian" == key){
								this.detail[key] = cross[key]
							}
					...
					this.huodongleixingOptions = "儿童娱乐部,舞会小剧场,杂技小剧场,亲子娱乐比赛,其他活动".split(',')
				},
				methods: {
					onPageTap(url) {
					...
					onConfirmTap(){
					if(!isIntNumer(this.detail.menpiaojiage)){
						alert(`门票价格应输入整数`);
						return
					}
						addFun("qinzihuodong", this.detail, (res) => {
							if(res&&res.code==0){
								alert("提交成功")
								window.history.go(-1);
							}
 			...
 			$(function(){
				...
				layui.use('upload', function(){
					...
					var uploadInst = upload.render({
						elem: '#huodongfengmianupload-img'
						...
						,before: function(obj){
						  ...
						  obj.preview(function(index, file, result){
							$('#huodongfengmianimg-temp').attr('src', result); //图片链接（base64）
						  });
						}
						,done: function(res){
						  //如果上传失败
						  if(res.code > 0){
							return layer.msg('上传失败');
							  ...
							  app.detail.huodongfengmian = baseurl+"/upload/"+res.file
				...
				layui.use('laydate', function(){
					...
					laydate.render({
						...
						type: 'date',
						    ...
						    app.detail.kaishishijian = value
						 }
					});
				});
				...
				layui.use('laydate', function(){
					...
					laydate.render({
						...
						type: 'date',
						    ...
						    console.log(value); //得到日期生成的值，如：2017-08-18
						    app.detail.jieshushijian = value
						 }
					});
				});
				...
				layui.use('layedit', function(){
				  ...
				  $("#btn-confirm").on("click", function(){
						app.detail.huodongxiangqing = layedit.getContent(index)
						alert("编辑成功")
				  })
				});
				layui.use(['form','layedit'], function(){
					...
					form.on('select(huodongleixing)', function(data){
						app.detail.huodongleixing = data.value
					});
				});
			});
		</script>
