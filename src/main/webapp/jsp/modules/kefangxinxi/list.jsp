...
<body>
									...
									<div id="add-row_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																																															<label>客房类型</label>
																<select name="kefangleixing" id="kefangleixingSelect">
																																</select>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="kefangbianhaoTh" class="sorting" onclick="sort('kefangbianhao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">客房编号</th>
																													...
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">客房照片</th>
																																																																	<th id="kefangmianjiTh" class="sorting" onclick="sort('kefangmianji')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">客房面积</th>
																																																			<th id="kefangloucengTh" class="sorting" onclick="sort('kefanglouceng')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">客房楼层</th>
																																																			<th id="mianfeikuandaiTh" class="sorting" onclick="sort('mianfeikuandai')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">免费宽带</th>
																																																			...
																																																			<th id="kefangjieshaoTh" class="sorting" onclick="sort('kefangjieshao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">客房介绍</th>
	...
	<script >
		...
		function search(){
			searchForm = { key: ""};
																								var kefangleixingIndex = document.getElementById("kefangleixingSelect").selectedIndex;
					var kefangleixingOptions = document.getElementById("kefangleixingSelect").options;
					var kefangleixingSelectedOption = kefangleixingOptions[kefangleixingIndex]
					if(kefangleixingSelectedOption.text != null &&  kefangleixingSelectedOption.text != ''){
						searchForm.kefangleixing = "%" + kefangleixingSelectedOption.text + "%";
					}		
																																																																																					getDataList();
		}
		// 获取数据列表
        function getDataList() {
			http("kefangxinxi/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
							...
							var kefangbianhaoCell  = document.createElement('td');

												 kefangbianhaoCell.innerHTML = item.kefangbianhao;

                
													row.appendChild(kefangbianhaoCell);
            				...
            				var kefangzhaopianCell  = document.createElement('td');

								                  var kefangzhaopianImg = document.createElement('img');
				  kefangzhaopianImg.setAttribute('src',item.kefangzhaopian);
				  kefangzhaopianImg.setAttribute('height','100px');
				  kefangzhaopianImg.setAttribute('width','100px');
				  kefangzhaopianCell.appendChild(kefangzhaopianImg);

				
													row.appendChild(kefangzhaopianCell);
				
            				var kefangmianjiCell  = document.createElement('td');

												 kefangmianjiCell.innerHTML = item.kefangmianji;

                
													row.appendChild(kefangmianjiCell);
				
            				var kefangloucengCell  = document.createElement('td');

												 kefangloucengCell.innerHTML = item.kefanglouceng;

                
													row.appendChild(kefangloucengCell);
				
            				var mianfeikuandaiCell  = document.createElement('td');

												  mianfeikuandaiCell.innerHTML = item.mianfeikuandai; 
				  
                
													row.appendChild(mianfeikuandaiCell);
            				...
            				var kefangjieshaoCell  = document.createElement('td');

												  //如果图文详情包含图片
				  if(item.kefangjieshao && item.kefangjieshao.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.kefangjieshao.indexOf('<img');
					var endIndex = item.kefangjieshao.indexOf('>');				
					kefangjieshaoCell.innerHTML = item.kefangjieshao.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.kefangjieshao != null && item.kefangjieshao != "" && item.kefangjieshao.length >= 11){
				  		kefangjieshaoCell.innerHTML = item.kefangjieshao.substring(0, 10) + "..."; 
				  	}else{
				  		kefangjieshaoCell.innerHTML = item.kefangjieshao;
				 	}
				  }

                
													row.appendChild(kefangjieshaoCell);
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.innerHTML = "增加"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","增加");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"zengjiafangyuan\")");
				if(crossBtnControl('增加') == true){
					btnGroup.appendChild(crossBtn);
				}   
				var crossBtn = document.createElement('button');
				crossBtn.innerHTML = "减少"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","减少");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"jianshaofangyuan\")");
				if(crossBtnControl('减少') == true){
					btnGroup.appendChild(crossBtn);
				}   
				var crossBtn = document.createElement('button');
				crossBtn.innerHTML = "预定"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","预定");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"kefangyuding\")");
				if(crossBtnControl('预定') == true){
					btnGroup.appendChild(crossBtn);
				}   
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("kefangxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
			...
			window.location.href = "info.jsp";
					...
					http("option/kefangleibie/kefangleixing","GET",{},(res)=>{
						if(res.code == 0){
							...
							kefangleixingSelect.appendChild(nullOption);
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"kefangxinxi");
