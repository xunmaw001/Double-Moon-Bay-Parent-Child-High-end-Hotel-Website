									...
									<div id="add-row_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																						<label>景点名称 <input type="text" id="jingdianmingchengSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																	<label>景点类型</label>
																<select name="jingdianleixing" id="jingdianleixingSelect">
																																	<option value=" "> </option>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="jingdianmingchengTh" class="sorting" onclick="sort('jingdianmingcheng')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">景点名称</th>
																																																			<th id="jingdianleixingTh" class="sorting" onclick="sort('jingdianleixing')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">景点类型</th>
																																																			
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">景点封面</th>
																																																																	<th id="kaifangshijianTh" class="sorting" onclick="sort('kaifangshijian')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">开放时间</th>
																																																			<th id="jingdiandizhiTh" class="sorting" onclick="sort('jingdiandizhi')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">景点地址</th>
																																																			<th id="jingdianjieshaoTh" class="sorting" onclick="sort('jingdianjieshao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">景点介绍</th>
	...
	<script >
		...
		function search(){
			searchForm = { key: ""};
						...
						searchForm.jingdianmingcheng ="%" +  $('#jingdianmingchengSearch').val() + "%";
																										...
																										var jingdianleixingIndex = document.getElementById("jingdianleixingSelect").selectedIndex;
					var jingdianleixingOptions = document.getElementById("jingdianleixingSelect").options;
					var jingdianleixingSelectedOption = jingdianleixingOptions[jingdianleixingIndex]
					if(jingdianleixingSelectedOption.text != null &&  jingdianleixingSelectedOption.text != ''){
						searchForm.jingdianleixing = "%" + jingdianleixingSelectedOption.text + "%";
					}		
																																																									getDataList();
		}
		// 获取数据列表
        function getDataList() {
			http("jiudianzhoubian/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																						jingdianmingcheng : searchForm.jingdianmingcheng,	
																																	jingdianleixing : searchForm.jingdianleixing,	
																																																																								},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var jingdianleixingCell  = document.createElement('td');

												  jingdianleixingCell.innerHTML = item.jingdianleixing; 
				  
                
													row.appendChild(jingdianleixingCell);
				
            				var jingdianfengmianCell  = document.createElement('td');

								                  var jingdianfengmianImg = document.createElement('img');
				  jingdianfengmianImg.setAttribute('src',item.jingdianfengmian);
				  jingdianfengmianImg.setAttribute('height','100px');
				  jingdianfengmianImg.setAttribute('width','100px');
				  jingdianfengmianCell.appendChild(jingdianfengmianImg);

				
													row.appendChild(jingdianfengmianCell);
            				...
            				var jingdianjieshaoCell  = document.createElement('td');
												  ...
												  //如果图文详情包含图片
				  if(item.jingdianjieshao && item.jingdianjieshao.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.jingdianjieshao.indexOf('<img');
					var endIndex = item.jingdianjieshao.indexOf('>');				
					jingdianjieshaoCell.innerHTML = item.jingdianjieshao.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.jingdianjieshao != null && item.jingdianjieshao != "" && item.jingdianjieshao.length >= 11){
				  		jingdianjieshaoCell.innerHTML = item.jingdianjieshao.substring(0, 10) + "..."; 
				  	}else{
				  		jingdianjieshaoCell.innerHTML = item.jingdianjieshao;
				 	}
				  }

                
													row.appendChild(jingdianjieshaoCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("jiudianzhoubian/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"jiudianzhoubian");
