...
<body>
									...
									<div id="add-row_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4 no-footer">
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																																															<label>设备名称 <input type="text" id="shebeimingchengSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																																																																			<label>设备状态</label>
																<select name="shebeizhuangtai" id="shebeizhuangtaiSelect">
																																	<option value=" "> </option>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="shebeibianhaoTh" class="sorting" onclick="sort('shebeibianhao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">设备编号</th>
																																																			<th id="shebeimingchengTh" class="sorting" onclick="sort('shebeimingcheng')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">设备名称</th>
																																																			
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">设备图片</th>
																																																																	<th id="shiyongquyuTh" class="sorting" onclick="sort('shiyongquyu')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">使用区域</th>
																																																			<th id="shebeizhuangtaiTh" class="sorting" onclick="sort('shebeizhuangtai')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">设备状态</th>
																																																			
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">设备详情</th>
	...
	<script >
		...
		function search(){
			searchForm = { key: ""};
																								if($('#shebeimingchengSearch').val() != null && $('#shebeimingchengSearch').val() != ''){
						searchForm.shebeimingcheng ="%" +  $('#shebeimingchengSearch').val() + "%";
					}				
																																								var shebeizhuangtaiIndex = document.getElementById("shebeizhuangtaiSelect").selectedIndex;
					var shebeizhuangtaiOptions = document.getElementById("shebeizhuangtaiSelect").options;
					var shebeizhuangtaiSelectedOption = shebeizhuangtaiOptions[shebeizhuangtaiIndex]
					if(shebeizhuangtaiSelectedOption.text != null &&  shebeizhuangtaiSelectedOption.text != ''){
						searchForm.shebeizhuangtai = "%" + shebeizhuangtaiSelectedOption.text + "%";
					}		
																						getDataList();
		}
		// 获取数据列表
        function getDataList() {
			http("shebeixinxi/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																															shebeimingcheng : searchForm.shebeimingcheng,	
																																																			shebeizhuangtai : searchForm.shebeizhuangtai,	
																											},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
							...
							var shebeibianhaoCell  = document.createElement('td');

												 shebeibianhaoCell.innerHTML = item.shebeibianhao;

                
													row.appendChild(shebeibianhaoCell);
				
            				var shebeimingchengCell  = document.createElement('td');

												 shebeimingchengCell.innerHTML = item.shebeimingcheng;

                
													row.appendChild(shebeimingchengCell);
				
            				var shebeitupianCell  = document.createElement('td');

								                  var shebeitupianImg = document.createElement('img');
				  shebeitupianImg.setAttribute('src',item.shebeitupian);
				  shebeitupianImg.setAttribute('height','100px');
				  shebeitupianImg.setAttribute('width','100px');
				  shebeitupianCell.appendChild(shebeitupianImg);

				
													row.appendChild(shebeitupianCell);
				
            				var shiyongquyuCell  = document.createElement('td');

												 shiyongquyuCell.innerHTML = item.shiyongquyu;

                
													row.appendChild(shiyongquyuCell);
				
            				var shebeizhuangtaiCell  = document.createElement('td');

												  shebeizhuangtaiCell.innerHTML = item.shebeizhuangtai; 
				  
                
													row.appendChild(shebeizhuangtaiCell);
				
            				var shebeixiangqingCell  = document.createElement('td');

												  if(item.shebeixiangqing != null && item.shebeixiangqing != "" && item.shebeixiangqing.length >= 11){
				  	shebeixiangqingCell.innerHTML = item.shebeixiangqing.substring(0, 10) + "..."; 
				  }else{
				  	shebeixiangqingCell.innerHTML = item.shebeixiangqing;
				  }

				
													row.appendChild(shebeixiangqingCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("shebeixinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"shebeixinxi");
