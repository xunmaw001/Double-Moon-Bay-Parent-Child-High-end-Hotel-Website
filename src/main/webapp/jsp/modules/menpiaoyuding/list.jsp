										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																						<label>用户名 <input type="text" id="yonghumingSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																																																																			<label>活动类型 <input type="text" id="huodongleixingSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="yonghumingTh" class="sorting" onclick="sort('yonghuming')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">用户名</th>
																																																			...
																																																			<th id="canjiarenshuTh" class="sorting" onclick="sort('canjiarenshu')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">参加人数</th>
	...
	<script >
		...
		function search(){
			searchForm = { key: ""};
						...
						searchForm.yonghuming ="%" +  $('#yonghumingSearch').val() + "%";
																																								...
																																								if($('#huodongleixingSearch').val() != null && $('#huodongleixingSearch').val() != ''){
						searchForm.huodongleixing ="%" +  $('#huodongleixingSearch').val() + "%";
					}				
																																																																							getDataList();
		}
		// 获取数据列表
        function getDataList() {
			http("menpiaoyuding/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var canjiarenshuCell  = document.createElement('td');

												 canjiarenshuCell.innerHTML = item.canjiarenshu;

                
													row.appendChild(canjiarenshuCell);
			...
			function sh(){
				...
				httpJson("menpiaoyuding/update","POST",data,(res)=>{
					if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("menpiaoyuding/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"menpiaoyuding");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"menpiaoyuding");
